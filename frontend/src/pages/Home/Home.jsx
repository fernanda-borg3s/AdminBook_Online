...
export default function Home() {
  ...
  todosLivros.forEach((livro) => {
    contagemFormatoLivro[livro.formato_livro] =
      (contagemFormatoLivro[livro.formato_livro] || 0) + 1;
  });
  ...
  //total do mes
  const nomeMesAtual = Object.keys(mesesMapeados).find(
    (key) => mesesMapeados[key] === String(mesAtual),
  );
  ...
  const filteredAllBook = todosLivros.filter((disponivel) => {
    return Object.values(disponivel).some(
      (value) =>
        typeof value === "string" && value.toLowerCase().includes(lowerBusca),
    );
  });
    ...
    <>
      <Container>
        ...
        <Row className="h-50 mb-4">
          <Col
            style={{ backgroundColor: "#fff", color: "#0e345a" }}
            className="p-3 me-1">
          ...
          <Col
            style={{ backgroundColor: "#0e345a", color: "#fff" }}
            className="p-3 me-1">
              ...
              <ul>
                <li>
                  <p className="fw-bold fs-3">
                    <i className="bi bi-1-square-fill pe-3"></i>
                    {labelsAutores[0]}
                  </p>
                  <span className="frequencia-autor fw-bold fs-3">
                    {dadosAutores[0]}
                  </span>
                </li>
                <li>
                  <p className="fw-bold fs-5">
                    {" "}
                    <i className="bi bi-2-square-fill pe-3"></i>
                    {labelsAutores[1]}
                  </p>
                  <span className="frequencia-autor fw-bold fs-5">
                    {dadosAutores[1]}
                  </span>
                </li>
                <li>
                  <p className="fw-bold fs-7">
                    <i className="bi bi-3-square-fill pe-3"></i>
                    {labelsAutores[2]}
                  </p>
                  <span className="frequencia-autor fw-bold fs-7">
                    {dadosAutores[2]}
                  </span>
                </li>
                <li>
                  <p>
                    <i className="bi bi-4-square pe-3"></i>
                    {labelsAutores[3]}
                  </p>
                ...
                <li>
                  <p>
                    <i className="bi bi-5-square pe-3"></i>
                    {labelsAutores[4]}
                  </p>
                ...
                <li>
                  <p>
                    <i className="bi bi-6-square pe-3"></i>
                    {labelsAutores[5]}
                  </p>
                ...
                <li>
                  <p>
                    <i className="bi bi-7-square pe-3"></i>
                    {labelsAutores[6]}
                  </p>
                ...
                <li>
                  <p>
                    <i className="bi bi-8-square pe-3"></i>
                    {labelsAutores[7]}
                  </p>
                ...
                <li>
                  <p>
                    <i className="bi bi-9-square pe-3"></i>
                    {labelsAutores[8]}
                  </p>
                ...
                <li>
                  <p>
                    <i className="pe-1 ten-square">10º </i>
                    {labelsAutores[9]}
                  </p>
        ...
        <Row className="tabela-container mb-4">
          ...
          <Search
            busca={busca}
            handleSearch={(ev) => setBusca(ev.target.value)}
          />
          <Tabela>
            {tableAllBookPaginatedData.map((livro, index) => {
              // Calcular o índice real
              const realIndex =
                (tableAllBookCurrentPage - 1) * ITEMS_PER_PAGE + index + 1;
              return (
                <tr key={index}>
                  <td>{realIndex}</td>
                  <td>{livro.titulo}</td>
                  <td>{livro.autor}</td>
                  <td>
                    {livro.data_mes === "Sem Registro"
                      ? " - "
                      : livro.data_mes + " de "}
                    {livro.data_ano === "Sem Registro" ? "-" : livro.data_ano}
                  </td>
                  <td>{livro.genero}</td>
