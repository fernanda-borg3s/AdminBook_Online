...
export default function LivroLidos() {
  ...
  const livrosLidosAnoData = labelLivrosLidosAno.map(
    (ano) => contagemPorAno[ano],
  );
  ...
  // Filtrando os livros lidos apenas em 2024
  livrosLidos.forEach((item) => {
    if (item.data_ano === String(anoAtual)) {
      contagemPorMes[item.data_mes] = (contagemPorMes[item.data_mes] || 0) + 1;
    }
  });
  ...
  for (let i = 0; i <= mesAtual; i++) {
    const mesNome = Object.keys(mesesMapeados).find(
      (key) => mesesMapeados[key] === String(i),
    );
    if (mesNome) {
  ...
  const mediaAnual = (
    (livrosLidos.length - livrosSemRegistroAno) /
    (labelLivrosLidosAno.length - 1)
  ).toFixed(1);

  //paginacao
  const [tableAllLidosCurrentPage, setTableAllLidosCurrentPage] = useState(1);
  const tableAllLidosTotalPages = Math.ceil(
    livrosLidos.length / ITEMS_PER_PAGE,
  );
        ...
        <Row className="tabela-container mb-4">
          ...
          <Tabela>
            {tableAllLidosPaginatedData.map((livro, index) => {
              const realIndex =
                (tableAllLidosCurrentPage - 1) * ITEMS_PER_PAGE + index + 1;
              return (
                <tr key={index}>
                  <td>{realIndex}</td>
                  <td>{livro.titulo}</td>
